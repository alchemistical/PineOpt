...
const BacktestInterface: React.FC<BacktestInterfaceProps> = ({
  onBacktestStart,
  onResultsReady
}) => {
  ...
  const [positionSize, setPositionSize] = useState(10); // Percentage
  const [commission, setCommission] = useState(0.1); // Percentage
      ...
      if (onResultsReady && result.success) {
        ...
        const transformedResult = {
          ...
          metrics: {
            ...
            sortino_ratio: 0, // Not provided by API
            ...
            winning_trades: 0, // Not provided directly
            losing_trades: 0, // Not provided directly
            avg_win: 0,
            avg_loss: 0,
            largest_win: 0,
            largest_loss: 0,
          ...
          trades: [], // Empty for now, would need separate API call
          equity_curve: [],
          drawdown_curve: [],
          price_data: [],
        ...
        const errorMsg = result.message || result.error || 'Backtest failed';
  ...
  // Check if selected strategy is Pine Script
  const isPineScript = selectedStrategyData?.language === 'pine';
      ...
      <div className="p-6 space-y-6">
              ...
              >
                ...
                {strategies.map((strategy) => (
                  <option key={strategy.id} value={strategy.id}>
                    {strategy.name} ({strategy.parameters_count} parameters)
                  </option>
                ))}
              </select>
              ...
              {selectedStrategyData && (
                <div className="mt-3 p-3 bg-gray-700 rounded-lg">
                  ...
                  <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                    ...
                    <span>{selectedStrategyData.parameters_count} parameters</span>
                    <div className="flex space-x-1">
                      {selectedStrategyData.tags.map(tag => (
                        <span key={tag} className="bg-gray-600 px-2 py-1 rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
        ...
        <div>
          ...
          <div className="mt-2 text-sm text-gray-400">
            {engine === 'pineopt' && '• Built-in engine with basic backtesting features'}
            {engine === 'jesse' && '• Advanced engine with institutional-grade features'}
          </div>
        </div>
        ...
        {isPineScript && (
          <div className="bg-yellow-900/30 border border-yellow-600/50 rounded-lg p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h4 className="text-yellow-200 font-medium">Pine Script Strategy Detected</h4>
                <p className="text-yellow-100/80 text-sm mt-1">
                  This strategy is written in Pine Script and needs to be converted to Python before backtesting.
                </p>
                <p className="text-yellow-100/80 text-sm mt-2">
                  <strong>Solution:</strong> Go to the AI Strategy Dashboard and use "Convert with AI" to convert this strategy to Python first.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex justify-center pt-4">
          <button
            ...
            disabled={backtestRunning || !selectedStrategy || isPineScript}
        ...
        {selectedStrategyData && (
          <div className="mt-6 p-4 bg-gray-700 rounded-lg">
            <h4 className="text-white font-medium mb-3">Backtest Configuration</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="text-white">{startDate} to {endDate}</div>
              </div>
            </div>
          </div>
        )}
