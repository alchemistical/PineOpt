...
const StrategyDashboard: React.FC<StrategyDashboardProps> = ({ 
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load strategies - Epic 7 API
      const strategiesResponse = await fetch('http://localhost:5007/api/v1/strategies/list?limit=50');
    ...
    switch (action) {
      ...
      case 'delete':
        if (confirm('Are you sure you want to delete this strategy?')) {
          try {
            await fetch(`http://localhost:5007/api/v1/strategies/${strategyId}`, { method: 'DELETE' });
